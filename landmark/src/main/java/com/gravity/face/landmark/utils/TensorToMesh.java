...
public final class TensorToMesh {
    ...
    public FaceMesh process(Size imageSize, TensorToMeshOptions options, float[][][][] rawScores, float[][][][] rawMeshes, RectF roi) {
        if (rawMeshes.length != 1 ||
                rawMeshes[0].length != 1 ||
                rawMeshes[0][0].length != 1 ||
                rawMeshes[0][0][0].length != options.getNumCoordinates() * options.getNumLandmarks())
            throw new IllegalArgumentException("RawMeshes dimension is not correct");
        if (rawScores.length != 1 || rawScores[0].length != 1 || rawScores[0][0].length != 1 || rawScores[0][0][0].length != 1)
